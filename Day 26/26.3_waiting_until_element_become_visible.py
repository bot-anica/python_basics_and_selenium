...
# ---------- ОЖИДАНИЕ ПОКА ЭЛЕМЕНТ НЕ СТАНЕТ ВИДИМЫМ ИСПОЛЬЗУЯ SELENIUM
print("----- ОЖИДАНИЕ ПОКА ЭЛЕМЕНТ НЕ СТАНЕТ ВИДИМЫМ ИСПОЛЬЗУЯ SELENIUM -----")
    ...
    while current_page < pages_quantity:
        try:
            ...
            element = (WebDriverWait(driver, 20).until(
                # visibility_of_element_located - ожидание появления элемента, который станет видимым
                EC.visibility_of_element_located((By.CSS_SELECTOR, ".category__show_more_btn"))
            ))
            element.click()
            current_page += 1
